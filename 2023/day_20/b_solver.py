...
hp_input_high_periodicity = {k: None for k in mod_inputs["hp"]}

for idx in range(1, 1000000000000000):
    pulses = [("button", "broadcaster", False)]

    num_rx_low_pulses = 0
    num_rx_high_pulses = 0

    while pulses and not all(v is not None for v in hp_input_high_periodicity.values()):
        # count pulses
        ...
        for pulse in pulses:
            ...
            if mod_id in hp_input_high_periodicity:
                if new_pulse_is_high:
                    hp_input_high_periodicity[mod_id] = idx
    ...
    if all(v is not None for v in hp_input_high_periodicity.values()):
        break

score = 1
for v in hp_input_high_periodicity.values():
    score *= v
