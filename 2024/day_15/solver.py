...
def parse_input(part):
    ...
    grid = {}
    for r_idx, row in enumerate(grid_txt.splitlines()):
        for c_idx, c in enumerate(row):
            if c != ".":
                grid[c_idx + 1j*r_idx] = c
...
def solve_pt2(grid, moves):
    ...
    for m_idx, m in enumerate(moves, 1):
        ...
        while curr_positions:
            ...
            if next_object == "#": # wall: don't move anything
                blocked = True
                break
            elif next_object in ("[", "]"): # box: keep track of box positions
                new_positions = {next_p, next_p + 1 if grid.get(next_p) == "[" else next_p - 1} - movable_box_positions
                curr_positions.update(new_positions)
                movable_box_positions.update(new_positions)
    ...
    score = sum(100*p.imag + p.real for p in grid if grid[p] == "[")
